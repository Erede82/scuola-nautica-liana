-- =============================================================================
-- P9E — Enforce A12 exam topic quotas in submit_exam_quiz_attempt
-- =============================================================================
-- Client-side pickExamQuestions already applies quotas; without server checks
-- an unlocked student can RPC-submit 20 easy exam-tagged questions and store
-- an official passed attempt.
--
-- Built on top of 20260725140000 (extensions.digest). Do not use bare digest():
-- pgcrypto lives in schema extensions on Supabase with search_path=public,pg_temp.
-- =============================================================================
...
BEGIN
  ...
  -- Quote topic A12: allineate a ExamQuizRules.a12TopicQuotas (non fidarsi del client).
...
COMMENT ON FUNCTION public.submit_exam_quiz_attempt(text, text, integer, boolean, jsonb) IS
  'Studente: submit atomico simulazione esame (20 domande). user_id da auth.uid(); conteggi/esito/snapshot server-side; quote topic A12; idempotente su (user_id, client_attempt_token).';

-- =============================================================================
-- P9E — Enforce A12 + D1 exam topic quotas after D1 rules RPC
-- =============================================================================
-- Client-side pickExamQuestions already applies quotas; without server checks
-- a modified client can submit any exam-tagged questions and persist an invalid
-- official attempt (wrong topic mix still marked passed/failed).
--
-- Built on top of 20260726120000 (D1 15/3 + extensions.digest). Do not use bare
-- digest(). Timestamp after 20260727120000 so D1 lesson trim stays earlier.
-- =============================================================================
...
BEGIN
  ...
  -- Quote topic A12: allineate a ExamQuizRules (a12TopicQuotas).
  IF v_category = 'A12' THEN
    IF
    (
      SELECT count(*) FILTER (
        WHERE upper(btrim(q.exam_topic_code)) = 'SCAFO'
      )::integer
      FROM tmp_exam_submit_answers t
    ...
    (
      SELECT count(*) FILTER (
        WHERE upper(btrim(q.exam_topic_code)) = 'MOTORE'
      )::integer
      FROM tmp_exam_submit_answers t
      INNER JOIN public.questions q ON q.id = t.question_id
    ) <> 1
    ...
    (
      SELECT count(*) FILTER (
        WHERE upper(btrim(q.exam_topic_code)) = 'SICUREZZA'
      )::integer
      FROM tmp_exam_submit_answers t
      INNER JOIN public.questions q ON q.id = t.question_id
    ) <> 3
    ...
    (
      SELECT count(*) FILTER (
        WHERE upper(btrim(q.exam_topic_code)) = 'MANOVRE'
      )::integer
      FROM tmp_exam_submit_answers t
      INNER JOIN public.questions q ON q.id = t.question_id
    ) <> 4
    ...
    (
      SELECT count(*) FILTER (
        WHERE upper(btrim(q.exam_topic_code)) = 'COLREG'
      )::integer
      FROM tmp_exam_submit_answers t
      INNER JOIN public.questions q ON q.id = t.question_id
    ) <> 2
    ...
    (
      SELECT count(*) FILTER (
        WHERE upper(btrim(q.exam_topic_code)) = 'METEO'
      )::integer
      FROM tmp_exam_submit_answers t
      INNER JOIN public.questions q ON q.id = t.question_id
    ) <> 2
    ...
    (
      SELECT count(*) FILTER (
        WHERE upper(btrim(q.exam_topic_code)) = 'NAV'
      )::integer
      FROM tmp_exam_submit_answers t
      INNER JOIN public.questions q ON q.id = t.question_id
    ) <> 4
    ...
    (
      SELECT count(*) FILTER (
        WHERE upper(btrim(q.exam_topic_code)) = 'NORM'
      )::integer
      FROM tmp_exam_submit_answers t
      INNER JOIN public.questions q ON q.id = t.question_id
    ) <> 3
    OR EXISTS (
      ...
      FROM tmp_exam_submit_answers t
      INNER JOIN public.questions q ON q.id = t.question_id
      WHERE upper(btrim(q.exam_topic_code)) NOT IN (
        'SCAFO', 'MOTORE', 'SICUREZZA', 'MANOVRE',
        'COLREG', 'METEO', 'NAV', 'NORM'
      )
    ) THEN
      RAISE EXCEPTION 'invalid_exam_topic_quotas';
    END IF;
  END IF;

  -- Quote topic D1: allineate a ExamQuizRules (d1TopicQuotas).
  IF v_category = 'D1' THEN
    IF
    (
      SELECT count(*) FILTER (
        WHERE upper(btrim(q.exam_topic_code)) = 'SCAFO'
...
COMMENT ON FUNCTION public.submit_exam_quiz_attempt(text, text, integer, boolean, jsonb) IS
  'Studente: submit atomico simulazione esame. A12=20/4, D1=15/3; quote topic server-side. user_id da auth.uid(); conteggi/esito/snapshot server-side; idempotente su (user_id, client_attempt_token).';

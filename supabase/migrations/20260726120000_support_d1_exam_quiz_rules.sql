-- =============================================================================
-- P9E-D1.1 — Supporto regole Quiz Esame D1 (15/3) oltre A12 (20/4)
-- =============================================================================
-- A12: 20 domande, max 4 errori (non risposte = errori), durata 30 min (client).
-- D1:  15 domande, max 3 errori (non risposte = errori), durata 30 min (client).
--
-- Non tocca: lesson quiz_results, Assigned Quiz, Contabilità, Stripe, payments.
-- Non modifica RLS/grants oltre il ripristino EXECUTE sulla RPC sostituita.
-- Mantiene extensions.digest (fix P9E.6-B).
-- =============================================================================

-- ---------------------------------------------------------------------------
-- CHECK header category-aware
-- ---------------------------------------------------------------------------
ALTER TABLE public.exam_quiz_attempts
  DROP CONSTRAINT IF EXISTS exam_quiz_attempts_total_questions_check;

ALTER TABLE public.exam_quiz_attempts
  DROP CONSTRAINT IF EXISTS exam_quiz_attempts_passed_counts_chk;

ALTER TABLE public.exam_quiz_attempts
  ADD CONSTRAINT exam_quiz_attempts_total_questions_by_category_chk CHECK (
    (license_category = 'A12' AND total_questions = 20)
    OR (license_category = 'D1' AND total_questions = 15)
  );

ALTER TABLE public.exam_quiz_attempts
  ADD CONSTRAINT exam_quiz_attempts_passed_counts_chk CHECK (
    (license_category = 'A12' AND passed = ((wrong_count + unanswered_count) <= 4))
    OR (license_category = 'D1' AND passed = ((wrong_count + unanswered_count) <= 3))
  );

COMMENT ON COLUMN public.exam_quiz_attempts.passed IS
  'Esito server-side: (wrong+unanswered) <= 4 per A12 (20 domande), <= 3 per D1 (15 domande).';

COMMENT ON COLUMN public.exam_quiz_attempts.total_questions IS
  '20 per A12, 15 per D1. Determinato server-side dalla categoria.';

-- Answers: position 1..20 resta un bound superiore sicuro (D1 usa 1..15 via RPC).
COMMENT ON TABLE public.exam_quiz_attempt_answers IS
  'Snapshot storico risposte esame. A12: 20 righe (pos 1-20). D1: 15 righe (pos 1-15).';
...
BEGIN
  ...
  -- Regole esame per categoria: A12 = 20/4, D1 = 15/3 (non risposte = errori).
  ...
  SELECT count(*)::integer,
         ...
         count(*) FILTER (
           WHERE position < 1
             OR position > v_expected_questions
         )::integer,
         count(*) FILTER (
  ...
  -- Regola superamento: errori = sbagliate + non risposte; soglia per categoria.
  v_passed := (v_wrong + v_unanswered) <= v_max_errors;
...
COMMENT ON FUNCTION public.submit_exam_quiz_attempt(text, text, integer, boolean, jsonb) IS
  'Studente: submit atomico simulazione esame. A12=20/4, D1=15/3. user_id da auth.uid(); conteggi/esito/snapshot server-side; idempotente su (user_id, client_attempt_token).';

-- =============================================================================
-- Scuola Nautica Liana — backoffice compatibility layer (existing Supabase DB)
-- =============================================================================
-- Safe to run on a NON-empty project that already has legacy tables such as
-- public.students, public.payments, quiz tables, etc.
--
-- Does NOT create public.students or public.payments (avoids clashing with
-- legacy shapes). Creates missing backoffice tables, extends public.students
-- with columns the Flutter app expects, and enables staff resolution via
-- public.school_user_roles + minimal RLS.
--
-- Does NOT enable RLS or add policies on public.payments (legacy).
-- =============================================================================

CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- ---------------------------------------------------------------------------
-- school_user_roles (staff / student link to auth)
-- ---------------------------------------------------------------------------
...
COMMENT ON TABLE public.school_user_roles IS
  'Ruoli applicativi: staff vs student. student_id valorizzato solo per role = student.';
...
-- ---------------------------------------------------------------------------
-- students — add columns expected by lib/data/supabase/dto/backoffice_rows.dart
-- (idempotent; no DROP/rewrite of legacy columns)
-- ---------------------------------------------------------------------------
...
ALTER TABLE public.students ADD COLUMN IF NOT EXISTS tax_code text;
ALTER TABLE public.students ADD COLUMN IF NOT EXISTS address jsonb;
...
ALTER TABLE public.students ADD COLUMN IF NOT EXISTS internal_notes text;
...
-- Gentle backfill for new columns when legacy rows have NULLs.
UPDATE public.students
SET enrolled_license_category = COALESCE(NULLIF(trim(enrolled_license_category), ''), 'motore')
WHERE enrolled_license_category IS NULL;

UPDATE public.students
SET registration_status = COALESCE(NULLIF(trim(registration_status), ''), 'active')
WHERE registration_status IS NULL;

UPDATE public.students s
SET enrolled_course_path = CASE s.enrolled_license_category
  WHEN 'motore' THEN 'entro_12_miglia'
  WHEN 'd1' THEN 'd1'
  WHEN 'vela' THEN 'entro_12_miglia_vela'
  ELSE 'entro_12_miglia'
END
WHERE s.enrolled_course_path IS NULL;
...
-- ---------------------------------------------------------------------------
-- Financial + study access + exams + notes + audit (no public.payments)
-- ---------------------------------------------------------------------------
...
-- Align CHECK with Dart BackofficeActivityType + SQL onboarding migration (idempotent).
...
-- ---------------------------------------------------------------------------
-- Row Level Security (new tables + students + school_user_roles; not payments)
-- ---------------------------------------------------------------------------
...
DROP POLICY IF EXISTS students_modify_staff ON public.students;
...
DROP POLICY IF EXISTS financial_select_policy ON public.student_financial_summaries;
...
DROP POLICY IF EXISTS financial_staff_all ON public.student_financial_summaries;
CREATE POLICY financial_staff_all ON public.student_financial_summaries
  FOR ALL USING (public.is_school_staff()) WITH CHECK (public.is_school_staff());

DROP POLICY IF EXISTS study_unlocks_select ON public.lesson_quiz_sheet_unlocks;
CREATE POLICY study_unlocks_select ON public.lesson_quiz_sheet_unlocks
  FOR SELECT USING (public.is_own_student(student_id) OR public.is_school_staff());

DROP POLICY IF EXISTS study_unlocks_staff_all ON public.lesson_quiz_sheet_unlocks;
CREATE POLICY study_unlocks_staff_all ON public.lesson_quiz_sheet_unlocks
  FOR ALL USING (public.is_school_staff()) WITH CHECK (public.is_school_staff());

DROP POLICY IF EXISTS exam_access_select ON public.exam_quiz_access;
CREATE POLICY exam_access_select ON public.exam_quiz_access
  FOR SELECT USING (public.is_own_student(student_id) OR public.is_school_staff());

DROP POLICY IF EXISTS exam_access_staff_all ON public.exam_quiz_access;
CREATE POLICY exam_access_staff_all ON public.exam_quiz_access
  FOR ALL USING (public.is_school_staff()) WITH CHECK (public.is_school_staff());

DROP POLICY IF EXISTS error_review_select ON public.error_review_topic_assignments;
CREATE POLICY error_review_select ON public.error_review_topic_assignments
  FOR SELECT USING (public.is_own_student(student_id) OR public.is_school_staff());

DROP POLICY IF EXISTS error_review_staff_all ON public.error_review_topic_assignments;
CREATE POLICY error_review_staff_all ON public.error_review_topic_assignments
  FOR ALL USING (public.is_school_staff()) WITH CHECK (public.is_school_staff());

DROP POLICY IF EXISTS guidance_select ON public.guidance_appointments;
CREATE POLICY guidance_select ON public.guidance_appointments
  FOR SELECT USING (public.is_own_student(student_id) OR public.is_school_staff());

DROP POLICY IF EXISTS guidance_staff_all ON public.guidance_appointments;
...
DROP POLICY IF EXISTS practice_staff_all ON public.practice_dossiers;
CREATE POLICY practice_staff_all ON public.practice_dossiers
  FOR ALL USING (public.is_school_staff()) WITH CHECK (public.is_school_staff());

DROP POLICY IF EXISTS staff_notes_staff_all ON public.staff_internal_notes;
CREATE POLICY staff_notes_staff_all ON public.staff_internal_notes
  FOR ALL USING (public.is_school_staff()) WITH CHECK (public.is_school_staff());

DROP POLICY IF EXISTS activity_staff_all ON public.backoffice_activity_events;
CREATE POLICY activity_staff_all ON public.backoffice_activity_events
  FOR ALL USING (public.is_school_staff()) WITH CHECK (public.is_school_staff());

DROP POLICY IF EXISTS school_roles_select_own ON public.school_user_roles;
...
-- =============================================================================
-- Fine migration compatibility backoffice
-- =============================================================================
